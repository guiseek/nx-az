import { coerceArray, mergeErrors, removeError } from './feat-forms.utils'
...
> extends NgFormGroup {
  ...
  select<R>(mapFn: (state: ControlsValue<T[]>) => R): Observable<R> {
    return selectControlValue$(this as FormGroup, mapFn as any)
  }
  ...
  get(path?: Array<string | number> | string) {
    return super.get(path as Array<string | number> | string)
  }
  ...
  patchValue(
    valueOrObservable: Observable<Partial<ControlsValue<T>>>,
    options?: ControlEventOptions
  ): Subscription
  patchValue(
    valueOrObservable: Partial<ControlsValue<T>>,
    options?: ControlEventOptions
  ): void
  patchValue(
    ...
    if (isObservable(valueOrObservable)) {
      return valueOrObservable.subscribe((value) =>
        super.patchValue(value as Record<string, any>, options)
      )
    }
  ...
  reset(
    formState?: Partial<ControlsValue<T>>,
    options?: ControlEventOptions
  ): void {
